...
void App::Update()
{
    ...
    // Metallic cube (shiny, fairly low roughness): almost no diffuse, a
    ...
    // artificially boosted specular color did.
    fillObjectConstants(m_mappedConstantBuffer, cubeWorld, 0.9f, 0.4f);
...
void App::Render()
{
    ...
    // Bound once for both draws below - unlike steps 4-13, switching which
    // texture a draw actually samples no longer means switching descriptor
    // tables, just the small root-constant index set before each draw.
    m_commandList->SetGraphicsRootDescriptorTable(2, m_srvHeap->GetGPUDescriptorHandleForHeapStart());

    const BindlessMaterialIndices cubeIndices = { 0, 2, 3, 0 }; // diffuse@0, normalMap@2, shadowMap@3
    m_commandList->SetGraphicsRoot32BitConstants(1, sizeof(cubeIndices) / sizeof(UINT32), &cubeIndices, 0);
