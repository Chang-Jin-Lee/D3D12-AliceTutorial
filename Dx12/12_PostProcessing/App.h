...
// Extends step 11 into real HDR post-processing: the main color pass now
// renders into a FLOAT16 target instead of an 8-bit UNORM one, so a bright
// specular highlight (boosted in Update()'s lightColor/specularColor)
// can carry color values above 1.0 instead of clipping to white. A
// bright-pass + repeated-blur chain (reusing step 11's blur shader) turns
// those over-bright pixels into a soft bloom halo, and a final tonemap
// pass (Reinhard) adds that halo back onto the scene and compresses the
// result down to the [0,1] range the back buffer can display. The shadow
// map, skybox, cube, and ground plane passes are otherwise unchanged from
// step 11.
class App
{
public:
