...
void App::Update()
{
    ...
    // A low shininess is used deliberately: with only 6 flat face normals
    // (no smooth interpolation), a tight, realistic highlight (e.g. 32+)
    // would only catch a face at very specific rotation angles and might
    // not be visible at all in a screenshot. A broad, soft highlight is
    // more reliably visible on flat-shaded geometry like this cube.
