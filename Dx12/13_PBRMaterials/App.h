...
// Extends step 12 by replacing the Blinn-Phong lighting model (steps 6-12)
// with a Cook-Torrance PBR (physically based rendering) BRDF driven by
// per-object metallic/roughness values instead of a fixed specular
// color/shininess. The cube is set to a shiny metal (metallic 1.0) and the
// ground plane to a duller non-metal (metallic 0.0) in Update(), so the two
// objects visibly demonstrate how those two parameters change a surface's
// look. Everything else - HDR target, bloom, tonemap, shadow map, skybox -
// is unchanged from step 12; only Shaders.hlsl's lighting math and the
// constant buffer's material fields changed.
class App
{
public:
