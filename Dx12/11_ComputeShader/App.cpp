...
void App::InitMsaaSupport()
{
    ...
    // swapchain's back-buffer format. Virtually all D3D12 hardware supports
    // 4x for a plain color format, but if it somehow doesn't, fall back to
    // 1x (no multisampling) rather than fail to create the resource below.
...
void App::InitComputePostProcess()
{
    ...
    // The image the compute shader reads: single-sample, so it's created
    ...
    resolvedDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    resolvedDesc.SampleDesc.Count = 1;

    ThrowIfFailed(m_device->CreateCommittedResource(
        &heapProps, D3D12_HEAP_FLAG_NONE, &resolvedDesc,
        D3D12_RESOURCE_STATE_RESOLVE_DEST, nullptr, IID_PPV_ARGS(&m_resolvedColorTarget)));

    // The image the compute shader writes: a resource must opt in with
    // ALLOW_UNORDERED_ACCESS before it can be bound as a UAV. Created
    // directly in UNORDERED_ACCESS for the same first-frame reason as above.
    D3D12_RESOURCE_DESC blurredDesc = resolvedDesc;
    blurredDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;

    ThrowIfFailed(m_device->CreateCommittedResource(
        &heapProps, D3D12_HEAP_FLAG_NONE, &blurredDesc,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&m_blurredColorTarget)));

    // One shader-visible heap holding both descriptors the blur shader
    // needs: the SRV it reads through (t0) followed by the UAV it writes
    // through (u0), matching the two-range compute root signature below.
    D3D12_DESCRIPTOR_HEAP_DESC computeHeapDesc = {};
    computeHeapDesc.NumDescriptors = 2;
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE computeHandle = m_computeHeap->GetCPUDescriptorHandleForHeapStart();
    ...
    m_device->CreateShaderResourceView(m_resolvedColorTarget.Get(), &srvDesc, computeHandle);
    computeHandle.ptr += descriptorSize;

    D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc = {};
    uavDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
    m_device->CreateUnorderedAccessView(m_blurredColorTarget.Get(), nullptr, &uavDesc, computeHandle);

    // Compute root signature: a single descriptor table with the SRV (t0)
    ...
    ThrowIfFailed(m_device->CreateRootSignature(
        0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&m_computeRootSignature)));
    ...
    if (FAILED(D3DCompileFromFile(
        L"BlurCompute.hlsl", nullptr, nullptr, "CSMain", "cs_5_0", compileFlags, 0, &computeShader, &error)))
    {
        throw std::runtime_error(error ? static_cast<const char*>(error->GetBufferPointer()) : "CSMain compile failed");
    }
...
void App::InitSkybox()
{
    ...
    psoDesc.NumRenderTargets = 1;
    psoDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
    // Same multisampled target as the main PSO - see InitPipelineState.
    psoDesc.SampleDesc.Count = m_msaaSampleCount;
    psoDesc.SampleDesc.Quality = m_msaaQualityLevel;
...
void App::Update()
{
    ...
    auto fillObjectConstants = [&](SceneConstantBuffer* cb, const XMMATRIX& world)
    {
        ...
        cb->specularColor = { 1.0f, 1.0f, 1.0f, 8.0f }; // .a = shininess
    };
...
void App::Render()
{
    ...
    // Pass 3: resolve the multisampled color target into an off-screen
    // single-sample texture (a compute shader can only read a plain
    // Texture2D SRV, not a multisampled one) instead of straight into the
    // back buffer as step 10 did.
    ...
    if (!m_isFirstFrame)
    {
        // m_resolvedColorTarget is already RESOLVE_DEST on frame 1 (see
        // InitComputePostProcess) - every frame after that, the compute
        // pass at the end of the previous frame left it as an SRV.
        preResolveBarriers[preResolveBarrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        preResolveBarriers[preResolveBarrierCount].Transition.pResource = m_resolvedColorTarget.Get();
    ...
    m_commandList->ResolveSubresource(
        m_resolvedColorTarget.Get(), 0, m_msaaColorTarget.Get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);
    ...
    postResolveBarriers[1].Transition.pResource = m_resolvedColorTarget.Get();
    ...
    // Pass 4: the compute-shader box blur. No vertices, no viewport, no
    // render target - just a root signature, a pipeline state, and a grid
    // of threads dispatched over the image.
    if (!m_isFirstFrame)
    {
        // m_blurredColorTarget is already UNORDERED_ACCESS on frame 1 (see
        // InitComputePostProcess) - every later frame left it as a copy
        // source after Pass 5 below.
        D3D12_RESOURCE_BARRIER blurredToUav = {};
        blurredToUav.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        blurredToUav.Transition.pResource = m_blurredColorTarget.Get();
        blurredToUav.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_SOURCE;
        blurredToUav.Transition.StateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
        blurredToUav.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        m_commandList->ResourceBarrier(1, &blurredToUav);
    }
    ...
    m_commandList->SetComputeRootDescriptorTable(0, m_computeHeap->GetGPUDescriptorHandleForHeapStart());
    ...
    // of 8 (BlurCompute.hlsl discards the extra off-edge threads itself).
    const UINT groupCountX = (m_width + 7) / 8;
    const UINT groupCountY = (m_height + 7) / 8;
    m_commandList->Dispatch(groupCountX, groupCountY, 1);

    // Pass 5: copy the blurred result into the current back buffer - a
    ...
    preCopyBarriers[0].Transition.pResource = m_blurredColorTarget.Get();
    ...
    m_commandList->CopyResource(m_renderTargets[m_frameIndex].Get(), m_blurredColorTarget.Get());
