...
void App::Render()
{
    ...
    // Resolve the multisampled color target down into the current back
    // buffer: average each pixel's N samples into the single sample the
    // swapchain actually needs, then hand that back buffer to Present.
    D3D12_RESOURCE_BARRIER preResolveBarriers[2] = {};
    preResolveBarriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    preResolveBarriers[0].Transition.pResource = m_msaaColorTarget.Get();
    ...
    preResolveBarriers[1].Transition.pResource = m_renderTargets[m_frameIndex].Get();
    ...
    m_commandList->ResourceBarrier(_countof(preResolveBarriers), preResolveBarriers);

    m_commandList->ResolveSubresource(
        m_renderTargets[m_frameIndex].Get(), 0, m_msaaColorTarget.Get(), 0, DXGI_FORMAT_R8G8B8A8_UNORM);

    D3D12_RESOURCE_BARRIER postResolveBarriers[2] = {};
    postResolveBarriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    postResolveBarriers[0].Transition.pResource = m_renderTargets[m_frameIndex].Get();
    ...
    postResolveBarriers[1].Transition.pResource = m_msaaColorTarget.Get();
